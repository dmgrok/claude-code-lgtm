...
export function parseRemoteSpec(specifier: string): RemoteSpec {
  ...
  if (parts.length < 2) {
    throw new Error(
      `Invalid remote specifier "${specifier}". Expected format: github:owner/repo[/subpath][#ref]`
    );
  }
...
export async function fetchPreset(spec: RemoteSpec, force = false): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `Network error fetching ${spec.owner}/${spec.repo}. Check your connection.\n${(err as Error).message}`
    );
  }
  ...
  if (res.status === 403) {
    throw new Error(
      `GitHub API rate limited for ${spec.owner}/${spec.repo}. Try again in a few minutes or use a local path.`
    );
  }
  if (!res.ok) {
  ...
  try {
    ...
    throw new Error(
      `No preset.json found in ${spec.owner}/${spec.repo}${hint}. ` +
