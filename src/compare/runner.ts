...
export const DEFAULT_PROMPT =
  'List every file in the current directory (not recursively). For each file, write one sentence describing what it does. Be terse.';
...
async function runClaude(
  ...
  const { stdout } = await execFileAsync(
    'claude',
    ['-p', prompt, '--output-format', 'json', '--max-budget-usd', '0.50', ...extraArgs],
...
export async function runBaseline(prompt?: string): Promise<SessionMetrics> {
  return runClaude(prompt ?? DEFAULT_PROMPT, ['--safe-mode'], 'baseline');
}

export async function runOptimized(prompt?: string): Promise<SessionMetrics> {
