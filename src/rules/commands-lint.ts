...
export class CommandsLintRule implements Rule {
  id = 'commands/lint';
  name = 'Commands Lint';
  ...
  private checkFrontmatter(filePath: string, content: string, findings: Finding[]): void {
  ...
  private checkBody(filePath: string, content: string, findings: Finding[]): void {
    const fmMatch = content.match(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/);
  ...
  private checkInjectionPatterns(filePath: string, content: string, findings: Finding[]): void {
