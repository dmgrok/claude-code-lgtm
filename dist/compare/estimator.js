...
export function estimateSavings(actual) {
    const assumptions = [];
    ...
    const estimatedUsage = {
        ...
        cache_creation_input_tokens: Math.round(actual.totalUsage.cache_creation_input_tokens * (1 - REDUCTION.cacheHitImprovement)),
        cache_read_input_tokens: Math.round(actual.totalUsage.cache_read_input_tokens * (1 + REDUCTION.cacheHitImprovement)),
    };
    const estimatedModel = 'opusplan';
    const estimatedCost = calculateCost(estimatedUsage, estimatedModel);
    assumptions.push(`Input tokens reduced ${(inputReduction * 100).toFixed(0)}% — permission pre-approval (${(REDUCTION.permissionRoundtrip * 100).toFixed(0)}%) + targeted reads (${(REDUCTION.readOptimization * 100).toFixed(0)}%)`);
    assumptions.push(`Cache hit rate improved ${(REDUCTION.cacheHitImprovement * 100).toFixed(0)}% from stable permission config`);
    ...
    const totalActualInput = actual.totalUsage.input_tokens +
        actual.totalUsage.cache_creation_input_tokens +
        actual.totalUsage.cache_read_input_tokens;
    const totalEstimatedInput = estimatedUsage.input_tokens +
    ...
    return {
        ...
        savings: {
            ...
            totalInputDeltaPct: totalActualInput > 0
                ? ((totalActualInput - totalEstimatedInput) / totalActualInput) * 100
                : 0,
            costDelta: actual.costUsd - estimatedCost,
            costDeltaPct: actual.costUsd > 0 ? ((actual.costUsd - estimatedCost) / actual.costUsd) * 100 : 0,
        },
        assumptions,
    };
}
